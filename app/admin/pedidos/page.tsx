...
export default function PedidosPage() {
   ...
   *
   * Excluídos permanecem no banco,
   * mas não entram nos indicadores
   * normais da página.
   */

  const pedidosAtivos = pedidos.filter(
    (pedido) => !pedido.excluido
  );
  ...
  const pedidosPendentes = pedidosAtivos.filter(
    (pedido) => pedido.status === "PENDENTE"
  );

  const pedidosEntregues = pedidosAtivos.filter(
  ...
  const pedidosFiltrados = pedidos.filter((pedido) => {
     ...
     *
     * EXCLUIDO:
     * mostra somente pedidos excluídos.
     *
     * Demais filtros:
     * mostram somente pedidos ativos
     * daquele status.
     */
    ...
    if (filtroStatus === "TODOS") {
      correspondeStatus = !pedido.excluido;
    } else if (filtroStatus === "EXCLUIDO") {
      correspondeStatus = pedido.excluido === true;
    } else {
      correspondeStatus =
        !pedido.excluido &&
   ...
   * pedidos excluídos não entram
   * no resumo normal.
   */

  const faturamento = pedidosAtivos.reduce(
    (total, pedido) =>
  ...
  useEffect(() => {
    ...
    const nivel = sessionStorage.getItem("adminNivel");
   ...
   * CARREGAR PEDIDOS
   *
   * Carregamos todos porque
   * precisamos permitir a consulta
   * dos excluídos pelo filtro.
   */
  ...
  async function handleExcluirPedido(pedido: any) {
    const confirmar = window.confirm(
      pedido.status === "ENTREGUE"
        ? `Deseja excluir o pedido #${pedido.id} da área de pedidos?\n\nComo este pedido já foi entregue, ele continuará registrado no histórico e no faturamento.`
        : `Deseja excluir o pedido #${pedido.id}?\n\nEle desaparecerá da página de pedidos e ficará registrado como excluído no histórico.`
    );
    ...
    try {
      ...
      alert(
        pedido.status === "ENTREGUE"
          ? "Pedido removido da área de pedidos. O histórico de faturamento foi preservado."
  ...
  if (acessoPermitido === false) {
    return (
      <AcessoRestritoPage
        onOk={() => window.location.replace("/admin")}
      />
    );
  }
  ...
  return (
    <main className="min-h-screen bg-[#111111] p-8 text-white">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="overflow-x-auto rounded-3xl border border-[#C8A95B]/20 bg-[#181818]">
                    ...
                    >
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          ...
                          {pedido.excluido ? (
                            <span className="rounded-lg border border-red-500/30 bg-red-500/10 px-4 py-2 text-sm font-semibold text-red-400">
                              Excluído
                            </span>
                          ) : (
                            <>
                              ...
                              <button
                                ...
                                className="rounded-lg border border-red-500/50 bg-red-500/10 px-4 py-2 text-sm font-semibold text-red-400 transition hover:bg-red-500 hover:text-white disabled:cursor-not-allowed disabled:opacity-50"
                              >
                                {excluindoId === pedido.id
                                  ? "Excluindo..."
                                  : "Excluir"}
                    ...
                    >
                      <div className="flex flex-col gap-5 md:flex-row">
                        ...
                        <div className="flex flex-1 flex-col">
                          ...
                          <div className="mt-6 space-y-3 border-t border-[#C8A95B]/10 pt-4">
