...
export default function ProductForm({
  ...
  setAltura(productToEdit.altura_do_calcanhar);
  setEstoque(String(productToEdit.estoque));
  setCores(productToEdit.cores);
  setTamanhos(productToEdit.tamanhos);

  let galeria: string[] = [];

try {
  galeria = productToEdit.galeria
  ...
  preco: Number(preco),
  material,
  altura_do_calcanhar: altura,
  tamanhos,
  cores,
  imagem_principal: imagens[0],
  galeria: JSON.stringify(imagens.slice(1)),
...
if (productToEdit) {
  console.log("ID DO PRODUTO:", productToEdit.id);
console.log("PRODUTO:", productToEdit);
  await updateProduct(productToEdit.id, dadosProduto);
} else {
  await createProduct(dadosProduto);
}
...
if (productToEdit && onCancelEdit) {
  ...
  return (
    <div className="rounded-3xl bg-[#181818] border border-[#C8A95B]/20 p-8">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <input
            ...
            className="w-full rounded-xl bg-[#111] border border-[#333] px-4 py-3"
          />
        </div>
        ...
        <div>
          <label className="block mb-2">Altura do salto</label>
          <input
            value={altura}
        ...
        <div>
          <label className="block mb-2">Tamanhos</label>
          <input
            value={tamanhos}
